...
- Source shape after this pass: 85 main Kotlin files, 35 JVM test files, 11 instrumented test files.
...
- `domain/usecase` now has production-used wrappers for spam checks, SMS checks, sync, blocklist management, and exports. They still delegate to `SpamRepository`; repository interfaces and model moves are next.
...
- Completed in this pass: roadmap 1.2.1, 1.2.2, 1.2.3, 1.2.4, 1.3.1, 1.3.2, 1.3.3, 1.3.4, 1.4.4, 1.4.5, and 1.5.1.
...
Roadmap 1.5.2 is next: extract `SpamCheckResult` and settings-facing models into `domain/model`.

Keep the next move narrow. `SpamCheckResult` is still declared at the bottom of `SpamRepository.kt` and is consumed by data checkers, services, UI screens, and tests. Move it without changing its fields, then update imports and rerun unit tests, Android-test compilation, ktlint, detekt, and Kover.
