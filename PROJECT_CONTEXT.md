...
- `di/DetectionModule` now provides the existing detection helper objects through Hilt, and `CheckerDependencies` routes the live checker chain plus screening-service/app-startup/worker helper calls through that seam.
...
- Completed in this pass: roadmap 1.2.1, 1.2.2, 1.2.3, 1.2.4, 1.3.1, 1.3.2, 1.3.3, 1.3.4, 1.4.4, 1.4.5, 1.5.1, 1.5.2, 1.5.3, 1.5.4, and 1.6.1 through 1.6.8. Roadmap 1.6.9 is WIP: the injectable helper seam is live, but the helper objects have not yet been converted into classes.
...
Roadmap 1.6.9 remains next: convert the remaining pure/object detection helpers (`SpamHeuristics`, `SmsContentAnalyzer`, `SpamMLScorer`, `CallbackDetector`, `SmsContextChecker`, `CampaignDetector`, `HashWildcardMatcher`) to constructor-injectable classes carefully, without changing hot-path behavior.

Keep the existing object facades and `SpamRepository.getInstance()` fallback until app, ViewModel, service, worker, UI utility, and test consumers are migrated one at a time.
