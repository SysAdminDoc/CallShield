...
- Source shape after this pass: 91 main Kotlin files, 35 JVM test files, 11 instrumented test files.
...
- `domain/repository` now defines spam-check, sync, and blocklist contracts. Use cases consume those interfaces through `data/repository/SpamRepositoryAdapter`, which delegates to the existing `SpamRepository`.
...
- Completed in this pass: roadmap 1.2.1, 1.2.2, 1.2.3, 1.2.4, 1.3.1, 1.3.2, 1.3.3, 1.3.4, 1.4.4, 1.4.5, 1.5.1, 1.5.2, and 1.5.3.
...
Roadmap 1.5.4 is next: split the non-detection concerns out of `SpamRepository`.

This is the first high-risk architecture task. Use the existing adapter and interfaces as the migration boundary, keep the 5-second screening path on one DataStore snapshot, and rerun the full integration set after each slice.
