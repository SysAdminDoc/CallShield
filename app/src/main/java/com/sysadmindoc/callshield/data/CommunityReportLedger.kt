 ...
 * number and vote type, kept as `number|type|epochMillis` strings.
 *
...
internal object CommunityReportLedger {
    ...
    fun contains(
        ...
        number: String,
        type: String,
    ): Boolean = entries.any { it.startsWith(keyOf(number, type) + SEPARATOR) }
    ...
    fun add(
        ...
        val next = entries + "${keyOf(number, type)}$SEPARATOR$now"
