    ...
    constructor() {
        ...
        fun matches(
            ...
            for (i in pattern.indices) {
                ...
                val n = number[i]
                if (p == '#') {
                    if (!n.isAsciiDigit()) return false
                } else {
                    if (p != n) return false
                }
            }
        ...
        fun coversOrCoveredBy(
            ...
            for (i in a.indices) {
                ...
                when {
                    ...
                    // position agrees
                    ca == '#' && cb.isAsciiDigit() -> bCoversA = false

                    // A wider at this pos
                    cb == '#' && ca.isAsciiDigit() -> aCoversB = false
        ...
        companion object {
             ...
             */
            val DEFAULT_COUNTRY_PREFIXES: List<String> =
                listOf(
                    "+1", // NANP (US, Canada)
                    "+33", // France
                    "+44", // UK
            ...
            fun matches(
                pattern: String,
                number: String,
            ): Boolean = shared.matches(pattern, number)

            fun matchesWithVariants(
                pattern: String,
                number: String,
                countryPrefixes: List<String> = DEFAULT_COUNTRY_PREFIXES,
            ): Boolean = shared.matchesWithVariants(pattern, number, countryPrefixes)

            internal fun numberVariants(
                number: String,
                countryPrefixes: List<String>,
            ): List<String> = shared.numberVariants(number, countryPrefixes)

            fun coveredNumberCount(pattern: String): Long = shared.coveredNumberCount(pattern)

            fun coversOrCoveredBy(
                a: String,
                b: String,
            ): Overlap = shared.coversOrCoveredBy(a, b)
        }
    }
