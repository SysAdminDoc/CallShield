...
private const val OPENPHISH_FEED_TTL_MILLIS = 6L * 60L * 60L * 1_000L
    ...
    override suspend fun lookup(
        ...
        return if (host != null && host in snapshot.hosts) {
            UrlThreatResult.malicious(
                ...
                ttlMillis = resultTtlMillis,
            )
        } else {
            UrlThreatResult.clean(source, sourceVersion, canonicalUrl, nowMillis, ttlMillis = resultTtlMillis)
        }
    }
            ...
            try {
                val request =
                    Request
                        ...
                        .header("User-Agent", URL_THREAT_USER_AGENT)
                        .apply { feedSnapshot?.etag?.let { header("If-None-Match", it) } }
                        .build()
                client.newCall(request).execute().use { response ->
                    if (response.code == 304) {
                        return@withContext feedSnapshot?.copy(expiresAtMillis = nowMillis + OPENPHISH_FEED_TTL_MILLIS)?.also {
                            feedSnapshot = it
                        }
                    }
