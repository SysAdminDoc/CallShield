...
class JsonParsingFuzzTest {
    ...
    private fun fuzzFindMatchingBracket(
        s: String,
        startIdx: Int,
        open: Char,
        close: Char,
    ...
    @Test
    fun `parse version with empty trees`() = fuzzParse("""{"version": 3, "model_type": "gbt", "trees": []}""")

    @Test
    fun `parse version 2 with no weights`() = fuzzParse("""{"version": 2}""")

    @Test
    fun `parse version 2 with empty weights array`() = fuzzParse("""{"version": 2, "weights": [], "bias": 0.0}""")

    @Test
    fun `parse version 2 with too few weights`() = fuzzParse("""{"version": 2, "weights": [1.0, 2.0, 3.0], "bias": -1.0}""")
    ...
    @Test
    fun `parse version 1`() = fuzzParse("""{"version": 1, "weights": [1.0], "bias": 0.0}""")

    @Test
    fun `parse version 99`() = fuzzParse("""{"version": 99, "model_type": "gbt", "trees": []}""")

    @Test
    fun `parse negative version`() = fuzzParse("""{"version": -1}""")

    @Test
    fun `parse version 0`() = fuzzParse("""{"version": 0}""")

    @Test
    fun `parse version as string`() = fuzzParse("""{"version": "three"}""")

    @Test
    fun `parse version very large`() = fuzzParse("""{"version": 999999999}""")

    // ── Deeply nested brackets ───────────────────────────────────────

    @Test
    fun `parse deeply nested open brackets`() = fuzzParse("[".repeat(100))

    @Test
    fun `parse deeply nested curly braces`() = fuzzParse("{".repeat(100))

    @Test
    fun `parse alternating brackets`() = fuzzParse("[{[{[{[{[{" + "}]}]}]}]}]")

    @Test
    fun `parse mismatched brackets`() = fuzzParse("""{"trees": [}]""")

    @Test
    fun `parse only closing brackets`() = fuzzParse("]]]]]]]]")

    @Test
    fun `parse 500 nested arrays`() = fuzzParse("[".repeat(500) + "]".repeat(500))

    // ── Malformed JSON ───────────────────────────────────────────────

    @Test
    fun `parse truncated JSON`() = fuzzParse("""{"version": 3, "model_type": "gbt", "trees": [{"fea""")

    @Test
    fun `parse missing closing brace`() = fuzzParse("""{"version": 3, "weights": [1.0, 2.0]""")

    @Test
    fun `parse missing closing bracket`() = fuzzParse("""{"version": 2, "weights": [1.0, 2.0}""")

    @Test
    fun `parse extra commas`() = fuzzParse("""{"version": 2,,, "weights": [1.0,,2.0,,], "bias": -1.0,}""")

    @Test
    fun `parse trailing garbage`() = fuzzParse("""{"version": 2, "weights": [1.0], "bias": 0.0} GARBAGE HERE""")

    @Test
    fun `parse leading garbage`() = fuzzParse("""GARBAGE {"version": 2}""")

    @Test
    fun `parse double colons`() = fuzzParse("""{"version":: 2}""")

    @Test
    fun `parse no quotes on keys`() = fuzzParse("""{version: 2, weights: [1.0]}""")

    @Test
    fun `parse single quotes`() = fuzzParse("""{'version': 2, 'weights': [1.0]}""")
    ...
    @Test
    fun `parse NaN in weights`() = fuzzParse("""{"version": 2, "weights": [NaN, NaN, NaN], "bias": NaN}""")

    @Test
    fun `parse Infinity in weights`() = fuzzParse("""{"version": 2, "weights": [Infinity, -Infinity], "bias": Infinity}""")

    @Test
    fun `parse very large numbers`() = fuzzParse("""{"version": 2, "weights": [1e308, -1e308, 1e-308], "bias": 1e999}""")

    @Test
    fun `parse negative zero`() = fuzzParse("""{"version": 2, "weights": [-0.0, -0.0], "bias": -0.0}""")

    @Test
    fun `parse integer overflow in weights`() = fuzzParse("""{"version": 2, "weights": [99999999999999999999999999999999], "bias": 0}""")

    // ── Injection attempts in JSON values ────────────────────────────

    @Test
    fun `parse SQL injection in string value`() = fuzzParse("""{"version": 2, "model_type": "'; DROP TABLE models;--"}""")

    @Test
    fun `parse script injection in value`() = fuzzParse("""{"version": 2, "model_type": "<script>alert(1)</script>"}""")

    @Test
    fun `parse null byte in JSON string`() = fuzzParse("{\"version\": 2, \"model_type\": \"gbt\u0000evil\"}")

    @Test
    fun `parse escape sequences in values`() = fuzzParse("""{"version": 2, "model_type": "gbt\n\t\r\\\""}""")

    @Test
    fun `parse unicode escapes in values`() = fuzzParse("""{"version": 2, "model_type": "\u0067\u0062\u0074"}""")
    ...
    @Test
    fun `parseGbtTrees trees not array`() = fuzzParseGbtTrees("""{"trees": "not_an_array"}""")

    @Test
    fun `parseGbtTrees trees with nested garbage`() = fuzzParseGbtTrees("""{"trees": [{"feature": [garbage]}]}""")

    @Test
    fun `parseGbtTrees empty string`() = fuzzParseGbtTrees("")

    @Test
    fun `parseGbtTrees trees key but no bracket after`() = fuzzParseGbtTrees("""{"trees" 42}""")
