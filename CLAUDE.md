...
**Released:** v1.4.0 (versionCode 20, backup format v2, Room DB v6)
...
- **v1.3.0** — Infrastructure upgrade, not another audit round. (1) New CrashReporter service — installs Thread.setDefaultUncaughtExceptionHandler in CallShieldApp.onCreate, serializes crashes to filesDir/crashes/crash_<epoch>.txt (bounded at 5 most recent), exposes Share Last Crash Log via a new QuickLink in MoreScreen. No off-device telemetry. file_paths.xml updated to expose the crashes dir through FileProvider. buildFeatures.buildConfig re-enabled so the crash header can read versionName/versionCode. (2) Instrumentation test harness added (androidTest source set, compose ui-test-junit4 + espresso-core + test.ext.junit deps). 3 suites: ThemePrimitivesTest (PremiumCard/SectionHeader/accentGlow render + click dispatch), DashboardStatusBadgeTest (DashboardStatusModel state machine), CrashReporterInstrumentedTest (end-to-end file IO + FileProvider share intent). (3) New instrumented.yml workflow runs connectedDebugAndroidTest on macos-latest emulator with KVM acceleration, AVD snapshot cached between runs. (4) HotPathBenchmarkTest asserts regression ceilings on WildcardRule.matches (10k iters < 500 ms), CampaignDetector record+check (1k < 200 ms), SpamMLScorer.score (1k < 300 ms), SpamHeuristics pure checks (1k < 100 ms). Ceilings tunable via -Dcallshield.benchHeadroom=2.0 for slow CI runners.
- **v1.4.0** — Five user-facing features. (A) Smart call labeling: new `CallCategory` enum (DebtCollector / Political / Robocall / Scam / Phishing / Telemarketer / Wangiri / Survey / Business / Unknown) + `CallCategoryResolver.resolve(SpamCheckResult)` prioritizing DB `type` tags > matchSource/layer > heuristic description keywords > confidence-gated ML. Conservative thresholds prevent mislabeling (ML-only hits <80% stay Unknown). Surfaced as AssistChip in NumberDetailScreen header + inline prefix on BlockedLogScreen rows. (B) Silent voicemail mode: new `KEY_SILENT_VOICEMAIL` DataStore key + `setSilentVoicemail()`; `CallShieldScreeningService.respondBlock` branches to `CallResponse.Builder().setSilenceCall(true)` when enabled, falls back to `.setDisallowCall(true).setRejectCall(true)` otherwise. Off by default. Settings toggle in Detection Engine card. (C) FTC fraud report: new `ReportFraudHelper.report(context, number, Authority)` — copies number to clipboard, opens `reportfraud.ftc.gov` or FCC fallback, Toast explains the paste step. Replaces the existing Do-Not-Call button in NumberDetailScreen. (D) Block reasoning narrative: new `BlockReasoning.explain(matchReason, description, confidence): Reasoning(headline, bullets)` reconstructs plain-English decision chain from persisted fields (no hot-path cost, no new DB columns). Shown under spam-score card in NumberDetailScreen. Covers all 15+ layers + allow-through sources. (E) Emergency contacts: `WhitelistEntry.isEmergency: Boolean` column added in **Room DB v6** with explicit `MIGRATION_5_6` (`ALTER TABLE whitelist ADD COLUMN isEmergency INTEGER NOT NULL DEFAULT 0`). SpamDao.getEmergencyContacts() Flow + setWhitelistEmergency() row update. `SpamRepository.isSpam` short-circuit now surfaces `emergency_contact` matchSource distinct from `manual_whitelist`. BlocklistScreen Whitelist tab renders emergency entries first with red PremiumCard accent + EMERGENCY badge. AddWhitelistDialog gains an "Emergency" checkbox that flips confirm-button color/label.
...
- `MainViewModel.blockedToday`/`blockedThisWeek`/`blockedLastWeek` must be built off the shared `timeAnchor` Flow (which re-emits every 60 s) via `flatMapLatest`. Do NOT revert to `repo.getBlockedCountSince(System.currentTimeMillis() - 86_400_000)` at VM construction — that bakes the cutoff into the Room query and the window never rolls forward while the process stays alive.
- `CallShieldApp.appScope` is a `lateinit` companion-object `CoroutineScope` populated in `onCreate()` for work that must outlive the component that launched it (the 10-second after-call feedback notification is the canonical user). Do NOT replace with the screening service's own Handler or scope — the system unbinds `CallScreeningService` shortly after `respondToCall()` returns and any pending work gets cancelled. Do NOT use for UI-scoped cancellable work.
- `SpamRepository.isSpam(..., realtimeCall: Boolean = true)` / `isSpamSms(..., realtimeCall: Boolean = true)` — the `realtimeCall` flag gates `CampaignDetector.recordCall()` and `showCallerIdOverlay()`. `CallLogScanner` and `SmsInboxScanner` pass `realtimeCall = false` so historical scans don't poison the in-memory campaign state (the singleton map would otherwise flag any NPA-NXX with 5+ historical unknowns as an active campaign, blocking real incoming calls from that prefix as `campaign_burst` for up to an hour).
- `SpamHeuristics.isInContacts` uses `cursor?.use { it.count > 0 } ?: false` — do NOT revert to manual `cursor?.close()` after `cursor?.count`, which leaks the cursor if `count` throws (e.g., remote-process crash during ContactsContract query).
- Every `client.newCall(request).execute()` in `data/remote/*.kt` and `CommunityContributor.kt` MUST be wrapped in `.use { response -> ... }` — the non-2xx early-return paths previously leaked the `Response` (and its underlying connection) back into OkHttp's pool until GC. `response.body?.string()` alone only closes the body, not the response.
- `LogExporter` prunes its own cache with `dir.listFiles { file -> file.name.startsWith("callshield_log_") }` — `cacheDir/exports` is shared with `BlocklistExporter` (which filters on `callshield_blocklist_`). Do NOT revert to `dir.listFiles()?.forEach { it.delete() }`, which nukes in-flight blocklist exports that are still being read by the receiving app via FileProvider.
- Room DB: **`DB_VERSION = 6`** as of v1.4.0. Migration `MIGRATION_5_6` adds `isEmergency INTEGER NOT NULL DEFAULT 0` to the `whitelist` table. `fallbackToDestructiveMigrationFrom(1,2,3,4)` is still in force, but v5→v6 goes through the explicit Migration object — do NOT add v5 to the destructive fallback list.
- `CallCategoryResolver` is deliberately conservative: anything short of an explicit `type` tag from the database OR a high-confidence layer signal (campaign_burst, sms_content, rcs_*) OR heuristic reasons with strong keywords (wangiri / premium / neighbor_spoof / rapid_fire / voip_spam_range) OR ML ≥80% confidence falls back to `CallCategory.Unknown`. Do NOT lower the 80% ML gate or add loose keyword matches — mislabeling a bank as "Scam" destroys user trust faster than missing a label does. Tests in `CallCategoryResolverTest` lock the priority order.
- `BlockReasoning.explain()` reconstructs the narrative from persisted `BlockedCall.matchReason` + `description` + `confidence` **at view time**, not at block time. Do NOT add a `reasoningChain` column to `BlockedCall` — that would add hot-path cost for every call. Just keep `matchReason` + `description` populated in `SpamRepository.logBlockedCall` and `explain()` will do the rest.
- `WhitelistEntry.isEmergency`: emergency-flagged rows surface as `matchSource = "emergency_contact"` in `SpamRepository.isSpam` (layer 1 allow-through). Do NOT collapse back to the single `"manual_whitelist"` matchSource — the log + reasoning panel use this distinction to render the red badge and the "always rings through" narrative.
- Silent voicemail mode: `CallResponse.Builder().setSilenceCall(true).setSkipCallLog(false)` routes blocked calls to voicemail. `setSilenceCall` was added in API 24 so it's safe on our minSdk=29. Leaving `setDisallowCall`/`setRejectCall` UNSET is intentional — those would hard-reject and cancel the silent-routing. Keep the two CallResponse branches fully separate.
