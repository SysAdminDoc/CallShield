...
- **One visual language.** Home, Activity, Lookup, Rules, and More now share the
  warm Signal Ledger layout, crisp dividers, and clearer status hierarchy.
- **Faster status reading.** Protection health, weekly outcomes, setup readiness,
  and database freshness are visible without opening another page.
- **Clearer number decisions.** Lookup and number details use concise verdicts,
  a visible risk gauge, and nearby block or report actions.
- **Better support pages.** Settings, statistics, onboarding, diagnostics, and
  release history use the same spacing and responsive row behavior.
...
- Shows instantly with the area code, then updates when SkipCalls answers. SkipCalls gives a verdict and a category but no report count. The number's detail page shows the category
...
| Strings | 1432 string resources and 33 plural groups (translation-ready) |
