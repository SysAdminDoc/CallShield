...
**Stack fingerprint:** AGP 8.10.1 · Kotlin 2.2.21 · Compose BOM 2026.05.00 · Room 2.8.4 · WorkManager 2.11.2 · OkHttp 5.3.2 · Moshi 1.15.1 · DataStore 1.2.1 · minSdk 29 · targetSdk 36 · KSP for Room codegen. **No Hilt, no KMP, no Glance, no SQLCipher.**
...
| 1.2.4 `HotListSyncWorker` — hot_numbers, hot_ranges, spam_domains parsed and stored, per-entry error tolerance | M | hot-feed source seam | `androidTest/.../HotListSyncTest.kt` |

`SpamRepository` now accepts a constructor-provided `AppDatabase` and `SpamDataSource`, so Android integration tests can use `Room.inMemoryDatabaseBuilder()` plus fake remote snapshots without starting the Hilt refactor. Remaining hot-list worker tests need a mockable hot-feed source seam in `HotDataSync`.

### 1.3 Compose UI Tests `[WIP]`

Six instrumented test files now exist (`CrashReporterInstrumentedTest`, `DashboardStatusBadgeTest`, `ThemePrimitivesTest`, `SpamPipelineIntegrationTest`, `SmsPipelineIntegrationTest`, `SyncIntegrationTest`). Remaining:

| Task | Size | Files |
|------|------|-------|
| 1.3.1 Onboarding flow — 4 pages, permission requests, call-screener setup | M | `androidTest/.../ui/OnboardingTest.kt` |
| 1.3.2 Dashboard — hero stats, sync freshness, screener banner | M | `androidTest/.../ui/DashboardTest.kt` |
| 1.3.3 Blocklist — add/delete, wildcard validation, swipe-to-delete + undo | L | `androidTest/.../ui/BlocklistTest.kt` |
| 1.3.4 Settings — toggle persistence, quiet-hours validation, schedule picker | M | `androidTest/.../ui/SettingsTest.kt` |

### 1.4 CI Pipeline `[DONE]` / `[NOW]` for coverage gate

`build.yml`, `instrumented.yml`, `merge-reports.yml`, `test.yml` exist. Outstanding:

| Task | Size | Files |
|------|------|-------|
| 1.4.4 Code-coverage gate via Kover, ratcheting threshold (start 35%, +5% per release) | M | `app/build.gradle.kts`, `.github/workflows/test.yml` |
| 1.4.5 Lint + ktlint + detekt as required PR checks | S | `.editorconfig`, `.github/workflows/test.yml` |
...
**Critical:** the call/SMS pipeline integration tests in 1.2.1/1.2.2 and sync integration tests in 1.2.3 now exist; add the hot-list integration tests before splitting repository sync responsibilities. Run the full integration set after each split to verify priority ordering and data-refresh behavior.
...
|---|---|---|
| Phase 1 — Foundation | 35 | ~57% done (1.1, 1.2.1, 1.2.2, 1.2.3, 1.4, 1.7.1 shipped; 1.2.4, 1.6, 1.7.2-7 open) |
