...
`domain/usecase` now contains thin, production-used wrappers: `CheckSpamUseCase`, `CheckSpamSmsUseCase`, `SyncDatabaseUseCase`, `ManageBlocklistUseCase`, and `ExportLogsUseCase`. The wrappers are wired into the live call/SMS screening paths, historical scanners, sync worker, and `MainViewModel` blocklist/export flows while `SpamRepository` remains the backing implementation. `domain/model` now owns `SpamCheckResult` and `SyncResult`. No standalone settings model existed to move in this step; DataStore keys/flows stay in `SpamRepository` until repository interfaces and splits land. This creates stable call sites for the later repository-interface and DI steps without changing the 5-second screening path.
...
| 1.5.3 Define repository interfaces in domain | M | `[NOW]` | `domain/repository/*.kt` |
| 1.5.4 Split `SpamRepository` into `SpamRepositoryImpl`, `SettingsRepository`, `SyncRepository`, `BlocklistRepository` | XL | `[NEXT]` | `data/repository/*.kt` |
